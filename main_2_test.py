...
# add cell layer1
    with tf.variable_scope('LSTM_cell'):
        ...
        with tf.name_scope('initial_state'):
            cell_init_state = bilstm_cell.zero_state(batch_size, dtype=tf.float32)
...
# add_output_layer(  ):
    ...
    with tf.variable_scope('out_hidden'):
        sentence_embedded = cell_outputs[:, -1, :]
        l_out_x = tf.reshape(sentence_embedded, [-1, cell_size], name='2_2D')
...
# add cell layer2
    with tf.variable_scope('LSTM_cell_2'):
        lstm_cell_2 = tf.contrib.rnn.BasicLSTMCell(cell_size_2, forget_bias=1.0, state_is_tuple=True)
        with tf.name_scope('initial_state_2'):
            cell_init_state_2 = lstm_cell_2.zero_state(batch_size_2, dtype=tf.float32)
        with tf.name_scope('RNN_2'):
            cell_outputs_2, cell_final_state_2 = tf.nn.dynamic_rnn(lstm_cell_2, l_in_y_2,
                                                                   initial_state=cell_init_state_2, time_major=False)
    # add_output_layer(  ):
    # shape = (batch * steps, cell_size)
    # shape = (batch * steps, cell_size)
    with tf.variable_scope('out_hidden_2'):
        doc_embedded = cell_outputs_2[:, -1, :]
    ...
    for j in range(6):
        ...
        for i in range(27):
            ...
            with tf.name_scope('cost'):
                cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=ys))
            ...
            if i == 0:
                ...
                feed_dict = {xs: word_embedded_train_batch, ys: one_hot_train_labels_batch,
                             cell_init_state: final_state, cell_init_state_2: final_state_2}


            _, cost, final_state, final_state_2, pred = sess.run(
                [train_op, cost, cell_final_state,cell_final_state_2, prediction],
                feed_dict=feed_dict)
