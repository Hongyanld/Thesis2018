...
def get_word2vec_model(text_series):
    if args.model:
        print("Loading word2vec models from", root_path + "model_2_test/")
        word_model = Word2Vec.load(root_path + "model_2_test/toefl11.word_model.bin")
    else:
...
def train_word2vec_model(text_series):
    ...
    print("\tSaving word2vec models to", root_path + "model_2_test/")
    word_model.save(root_path + "model_2_test/toefl11.word_model.bin")
...
def train_word_vector(word_model):
    ...
    print("\tSaving word_vector to", root_path + "model_2_test/")
    with open(root_path + "model_2_test/"  + "text_series.word_vector.pickle", "wb") as file_to_write:
        pickle.dump(word_vector, file_to_write, protocol=4)
    return word_vector

def get_word_vector(word_model):
    if args.word_vector: #?
        print("Loading word_vector from", root_path + "model_2_test/")
        with open(root_path + "model_2_test/"  + "text_series.word_vector.pickle", "rb") as file_to_read:
...
def get_feature_matrix(text_series, word_indices):
    if args.feature_matrix:
        print("Loading feature matrices from", root_path + "model_2_test/")
        with open(root_path + "model_2_test/" + args.data_file + ".word_sequence_matrix.pickle", "rb") as file_to_read:
        ...
        with open(root_path + "model_2_test/" + args.data_file + ".word_sequence_matrix.pickle", "wb") as file_to_write:
...
#         print("Loading text_word_sequence_embedded from", root_path + "model_2_test/")
#         with open(root_path + "model_2_test/" + args.data_file + ".doc_sequence_embedded.pickle", "rb") as file_to_read:
...
#         with open(root_path + "model_2_test/" + args.data_file + ".doc_sequence_embedded.pickle", "wb") as file_to_write:
...
def main():
    ...
    df_train, df_test = train_test_split(df, stratify=df[['Language']],  # , 'Prompt'
...
# add cell layer1
    with tf.variable_scope('LSTM_cell'):
        fw_lstm_cell = tf.contrib.rnn.BasicLSTMCell(cell_size, forget_bias=1.0, state_is_tuple=True)
        bw_lstm_cell = tf.contrib.rnn.BasicLSTMCell(cell_size, forget_bias=1.0, state_is_tuple=True)
        # with tf.name_scope('initial_state'):
        #     cell_init_state_fw = fw_lstm_cell.zero_state(batch_size, dtype=tf.float32)
        #     cell_init_state_bw = bw_lstm_cell.zero_state(batch_size, dtype=tf.float32)
        with tf.name_scope('RNN'):
            ((outputs_fw, outputs_bw), (outputs_state_fw, outputs_state_bw)) = tf.nn.bidirectional_dynamic_rnn(
                fw_lstm_cell, bw_lstm_cell, l_in_y,dtype=tf.float32,time_major=False)
            ...
            outputs_final_state = tf.contrib.rnn.LSTMStateTuple(c=final_state_c,
                                                                h=final_state_h)
            final_state_fw = tf.concat((outputs_state_fw.c, outputs_state_fw.h), 1)
            final_state_bw = tf.concat((outputs_state_bw.c, outputs_state_bw.h), 1)
...
# add cell layer2
    with tf.variable_scope('LSTM_cell_2'):
        fw_lstm_cell_2 = tf.contrib.rnn.BasicLSTMCell(cell_size_2, forget_bias=1.0, state_is_tuple=True)
        bw_lstm_cell_2 = tf.contrib.rnn.BasicLSTMCell(cell_size_2, forget_bias=1.0, state_is_tuple=True)
        # with tf.name_scope('initial_state_2'):
        #     cell_init_state_fw_2 = fw_lstm_cell_2.zero_state(batch_size_2, dtype=tf.float32)
        #     cell_init_state_bw_2 = bw_lstm_cell_2.zero_state(batch_size_2, dtype=tf.float32)
        with tf.name_scope('RNN_2'):
            ((outputs_fw_2, outputs_bw_2), (outputs_state_fw_2, outputs_state_bw_2))  = tf.nn.bidirectional_dynamic_rnn(
                fw_lstm_cell_2, bw_lstm_cell_2, l_in_y_2, dtype=tf.float32,time_major=False)
            outputs_2 = tf.concat((outputs_fw_2, outputs_bw_2), 2)
            # final_state_c_2 = tf.concat((outputs_state_fw_2.c, outputs_state_bw_2.c), 1)
            # final_state_h_2 = tf.concat((outputs_state_fw_2.h, outputs_state_bw_2.h), 1)
            # outputs_final_state_2 = tf.contrib.rnn.LSTMStateTuple(c=final_state_c_2,
            #                                                     h=final_state_h_2)
            # final_state_fw_2 = tf.concat((outputs_state_fw_2.c, outputs_state_fw_2.h), 1)
            # final_state_bw_2 = tf.concat((outputs_state_bw_2.c, outputs_state_bw_2.h), 1)
    # with tf.variable_scope('Max_Pooling_2'):
        # doc_embedded = GlobalMaxPool1D()(outputs_2)
        # doc_embedded = GlobalAveragePooling1D(outputs_2)
        # doc_embedded = attention(out_put, self.attention_dim, self.l2_reg_lambda)
    ...
    for j in range(1):
        ...
        for k in range(2):
            ...
            with tf.name_scope("accuracy_val"):
                ...
                print('correct_num_val_all', correct_num_val_all)
