...
def build_pos_sequence_lstm_model(pos_indices, pos_vectors, output_dim):
    ...
    bi_lstm_pos_sent = Bidirectional(
        LSTM(lstm_unit_size, return_sequences=True, kernel_regularizer=regularizers.l2(0.01),
             recurrent_regularizer=regularizers.l1(0.01), bias_regularizer=regularizers.l2(0.01), dropout=lstm_dropout,
             recurrent_dropout=lstm_dropout))(
        pos_embedded)

    bi_lstm_pos_sent = GlobalMaxPool1D()(bi_lstm_pos_sent)
    ...
    b_lstm_pos_doc = Bidirectional(
        LSTM(lstm_unit_size, return_sequences=True, kernel_regularizer=regularizers.l2(0.01),
             recurrent_regularizer=regularizers.l1(0.01), bias_regularizer=regularizers.l2(0.01), dropout=lstm_dropout,
             recurrent_dropout=lstm_dropout))(pos_encoded)

    b_lstm_pos_sent = GlobalMaxPool1D()(b_lstm_pos_sent)
...
def build_word_sequence_lstm_model(word_indices, word_vectors, output_dim):
 ...
 # from word_indices of all word vocabulary to word_embedded for one sentence of text
    ...
    bi_lstm_word_sent = Bidirectional( LSTM(lstm_unit_size, return_sequences=True, kernel_regularizer=regularizers.l2(0.01),
             recurrent_regularizer=regularizers.l1(0.01), bias_regularizer=regularizers.l2(0.01), dropout=lstm_dropout,
             recurrent_dropout=lstm_dropout))(word_embedded)

    bi_lstm_word_sent = GlobalMaxPool1D()(bi_lstm_word_sent)
    ...
    b_lstm_word_doc = Bidirectional(LSTM(lstm_unit_size, return_sequences=True, kernel_regularizer=regularizers.l2(0.01),
             recurrent_regularizer=regularizers.l1(0.01), bias_regularizer=regularizers.l2(0.01), dropout=lstm_dropout,
             recurrent_dropout=lstm_dropout))(word_encoded)

    b_lstm_word_doc = GlobalMaxPool1D()(b_lstm_word_doc)


    word_output = Dropout(dropout, name='final_layer_word')(b_lstm_word_doc)#(attention_doc)
    word_output = Dense(output_dim, activation='softmax',kernel_regularizer=regularizers.l2(0.01),
                activity_regularizer=regularizers.l1(0.01))(word_output)#(word_output)
    ...
    #
    # original_cost_function = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=word_output, labels=ys))
    #
    # lost = original_cost_function + regularization_cost
    #
...
def myLossFunction(y_true, y_pred):
    # Return a tensor object from Lamada * tf.reduce_sum([tf.nn.l2_loss(v) for v in tv])
    return
